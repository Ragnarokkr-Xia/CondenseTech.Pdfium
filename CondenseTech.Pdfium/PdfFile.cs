...
namespace CondenseTech.Pdfium
{
    internal class PdfFile : IDisposable
    {
        ...
        public PdfPageLinks GetPageLinks(int pageNumber, SKSize pageSize)
        {
            if (_disposed)
                throw new ObjectDisposedException(GetType().Name);
                ...
                while (NativeMethods.FPDFLink_Enumerate(pageData.Page, ref link, out annotation))
                {
                    ...
                    if (NativeMethods.FPDFLink_GetAnnotRect(annotation, rect) && (target.HasValue || uri != null))
                    {
                        links.Add(new PdfPageLink(
                            new SKRect(rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top),
                            target,
                            uri
                        ));
                    }
        ...
        public List<SKSize> GetPDFDocInfo()
        {
            if (_disposed)
                throw new ObjectDisposedException(GetType().Name);

            int pageCount = NativeMethods.FPDF_GetPageCount(_document);
            var result = new List<SKSize>(pageCount);
        ...
        public SKSize GetPDFDocInfo(int pageNumber)
        {
            double height;
            double width;
        ...
        public SKPointI PointFromPdf(int page, SKPoint point)
        {
            using (var pageData = new PageData(_document, _form, page))
            {
                ...
                return new SKPointI(deviceX, deviceY);
            }
        }
        ...
        public SKPoint PointToPdf(int page, SKPointI point)
        {
            using (var pageData = new PageData(_document, _form, page))
            {
                ...
                return new SKPoint((float)deviceX, (float)deviceY);
            }
        }
        ...
        private SKRect GetBounds(IntPtr textPage, int index)
        {
